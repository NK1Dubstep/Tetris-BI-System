...
(cobra bots) new cobra metrics bot to send data to progd using Python for processing the message

## [9.8.1] - 2020-06-19

(cobra metrics to progd bot) fps slow frame info : do not include os name

## [9.8.0] - 2020-06-19

(cobra metrics to progd bot) send info about memory warnings
...
(cobra metrics to progd bot) send info about net requests
...
(cobra metrics to progd bot) change from a progd type of gauge to a timing one
...
(progd cobra bots) progd improvement: prefix does not need a dot as a suffix, message size can be larger than 256 bytes, error handling was invalid, use core logger for logging instead of std::cerr
...
(cobra to progd bot) add ability to extract a numerical value and send a timer event to progd, with the --timer option

## [9.1.8] - 2020-03-29

(cobra to progd bot) bot init was missing + capture socket error

## [9.1.7] - 2020-03-29

(cobra to progd bot) add ability to extract a numerical value and send a gauge event to progd, with the --gauge option
...
(cobra to progd) port to windows and add a unittest
...
(ws cobra to progd and sentry sender) exit if no messages are received for one minute, which is a sign that something goes wrong on the server side. That should be changed to be configurable in the future
...
(ws cobra to sentry/progd) fix for handling null events properly for empty queues + use queue to send data to progd
...
- ws cobra_subscribe knows to and display msg prog (count and # of messages received per second)
- ws cobra_subscribe, cobra_to_progd and cobra_to_sentry commands have a new option, --filter to restrict the events they want to receive
